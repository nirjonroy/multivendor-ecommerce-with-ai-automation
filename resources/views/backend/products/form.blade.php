...
@section('content')
    <form class="content-card" method="POST" action="{{ $product ? route('admin.products.update', $product) : route('admin.products.store') }}" enctype="multipart/form-data">
        @csrf
        @if($product)
            @method('PUT')
        @endif
        ...
        <div class="row">
            <div class="col-md-3">
                <label>Thumbnail Image Preview</label><br>
        ...
        <div class="row mt-4">
            <div class="col-md-6 form-group"><label>Name *</label><input class="form-control" name="name" value="{{ old('name', $product?->name) }}" required></div>
            <div class="col-md-6 form-group"><label>Video Url</label><input class="form-control" name="video_url" value="{{ old('video_url', $product?->video_url) }}"></div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-4 form-group"><label>SKU</label><input class="form-control" name="sku" value="{{ old('sku', $product?->sku) }}"></div>
            <div class="col-md-4 form-group"><label>Slug</label><input class="form-control" name="slug" value="{{ old('slug', $product?->slug) }}" placeholder="Auto generated if empty"></div>
        </div>
        ...
        <div class="row">
            <div class="col-md-4 form-group">
                ...
                <select class="form-control" name="owner_type">
                    <option value="admin" @selected(old('owner_type', $product?->owner_type ?? 'admin') === 'admin')>Admin</option>
                    <option value="vendor" @selected(old('owner_type', $product?->owner_type) === 'vendor')>Vendor</option>
                </select>
            </div>
            <div class="col-md-4 form-group">
                <label>Vendor</label>
                <select class="form-control" name="vendor_id">
                    ...
                    @foreach($vendors as $vendor)
                        <option value="{{ $vendor->id }}" @selected(old('vendor_id', $product?->vendor_id) == $vendor->id)>{{ $vendor->name }} - {{ $vendor->email }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4 form-group">
        ...
        <div class="row">
            <div class="col-md-4 form-group"><label>Price *</label><input class="form-control" type="number" step="0.01" name="price" value="{{ old('price', $product?->price) }}" required></div>
            <div class="col-md-4 form-group"><label>Offer Price</label><input class="form-control" type="number" step="0.01" name="offer_price" value="{{ old('offer_price', $product?->offer_price) }}"></div>
            <div class="col-md-4 form-group"><label>Stock Quantity *</label><input class="form-control" type="number" name="stock_quantity" value="{{ old('stock_quantity', $product?->stock_quantity ?? 0) }}" required></div>
        </div>

        <div class="form-group"><label>Short Description</label><textarea class="form-control" name="short_description" rows="2">{{ old('short_description', $product?->short_description) }}</textarea></div>
        <div class="form-group"><label>Long Description *</label><textarea class="form-control" id="editor1" name="long_description" rows="8">{{ old('long_description', $product?->long_description) }}</textarea></div>

        <div class="row">
            <div class="col-md-4 form-group"><label>Product Size and price</label><select class="form-control"><option>Variable Size</option></select></div>
            <div class="col-md-4 form-group"><label>Product Color and Image</label><select class="form-control"><option>Variable Color</option></select></div>
            <div class="col-md-4 form-group">
                <label>Variation Stock</label>
                <select class="form-control" name="has_variation_stock">
                ...
                <table class="table variation-table" id="size-table">
                    ...
                    <tbody>
                        @foreach($sizeRows as $i => $row)
                            <tr>
                                <td><select class="form-control" name="size_prices[{{ $i }}][size]">@foreach($productSizes as $size)<option value="{{ $size->name }}" @selected(($row['size'] ?? '') === $size->name)>{{ $size->name }}</option>@endforeach</select></td>
                ...
                <table class="table variation-table" id="color-table">
                    ...
                    <tbody>
                        @foreach($colorRows as $i => $row)
                            <tr>
                                <td><select class="form-control" name="color_images[{{ $i }}][color]">@foreach($productColors as $color)<option value="{{ $color->name }}" @selected(($row['color'] ?? '') === $color->name)>{{ $color->name }}</option>@endforeach</select></td>
        ...
        <div class="mt-3">
            <label>Manage Variation Quantity</label>
...
@push('scripts')
    ...
    <script>
        ...
        let stockIndex = {{ count($stockRows) }};
