...
<head>
    ...
    <style>
        ...
        .table th{background:#f8fafc}.print-actions{text-align:right;margin:20px auto;max-width:980px}
        @media print{body{background:#fff}.print-actions{display:none}.invoice-wrap{margin:0;border:0;max-width:100%}}
    </style>
</head>
<body>
@php($vendorTotal = $order->items->sum('subtotal'))
<div class="print-actions">
    <button class="btn btn-primary" onclick="window.print()">Print Invoice</button>
    <a class="btn btn-outline-secondary" href="{{ route('vendor.orders.show', $order) }}">Back</a>
</div>
<div class="invoice-wrap">
    <div class="invoice-head">
        <div>
            <h3>{{ $vendor->shop_name ?: $vendor->name }}</h3>
            <p class="mb-0">{{ $vendor->shop_address ?: $vendor->phone }}</p>
            <p class="mb-0">{{ $vendor->shop_email ?: $vendor->email }} {{ $vendor->shop_phone ? ' | ' . $vendor->shop_phone : '' }}</p>
        </div>
        <div class="text-right">
            ...
            <p class="mb-0"><strong>Total:</strong> {{ \App\Support\Currency::format($vendorTotal, $globalSiteInfo) }}</p>
        </div>
    </div>
    <div class="row mb-4">
        <div class="col-md-6">
    ...
    <table class="table table-bordered">
        <thead><tr><th>Product</th><th>Options</th><th>Status</th><th>Qty</th><th>Price</th><th>Subtotal</th></tr></thead>
        <tbody>
        ...
        <tfoot><tr><th colspan="5" class="text-right">Vendor Total</th><th>{{ \App\Support\Currency::format($vendorTotal, $globalSiteInfo) }}</th></tr></tfoot>
    </table>
</div>
</body>
</html>
